...
describe("findFirstNMatches", () => {
	...
	test("returns correct matches for standard input", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["apple", "apricot", "apex"]);
	});
	...
	test("returns all matches when exact number of matches are requested", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["apple", "apricot", "appraisal"]);
	});
	...
	test("returns only available matches when more matches are requested", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["apple", "apricot"]);
	});
	...
	test("is case-sensitive", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["Apple", "Apex"]);
	});
	...
	test("returns matches for empty prefix", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["apple", "banana"]);
	});
	...
	test("handles large dataset efficiently", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["apple"]);
	});
	...
	test("handles special characters in prefix and words", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["@pple"]);
	});
	...
	test("only matches words starting with prefix", () => {
		...
		expect(findFirstNMatches(words, prefix, numMatches)).toStrictEqual(["append"]);
	});
});
