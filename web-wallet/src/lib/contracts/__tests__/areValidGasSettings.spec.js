...
describe("areValidGasSettings", () => {
	...
	it("should validate the provided gas limit and gas price based on the boundaries", () => {
		expect(areValidGasSettings(realStore.gasPriceLower, realStore.gasLimitUpper)).toBe(true);

		expect(areValidGasSettings(realStore.gasPriceLower, realStore.gasLimitLower)).toBe(true);

		expect(areValidGasSettings(realStore.gasPriceLower + 1, 0)).toBe(false);

		expect(areValidGasSettings(realStore.gasPriceLower, realStore.gasLimitLower - 1)).toBe(false);

		expect(areValidGasSettings(realStore.gasPriceLower - 1, realStore.gasLimitUpper * 2)).toBe(false);
