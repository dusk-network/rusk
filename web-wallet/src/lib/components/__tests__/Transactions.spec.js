...
describe("Transactions", () => {
	...
	const blockExplorerBaseUrl = "https://explorer.dusk.network/transactions/transaction?id=";
	...
	it("renders transactions correctly when items are fulfilled", async () => {
		...
		const transactionBlockHeight =
			getByText(new Intl.NumberFormat(language).format(transaction.block_height));
	...
	it("renders the Transactions in descending order", async () => {
		...
		const transactionElements = container.querySelectorAll(".transactions-list");
	...
	it("renders the Transactions with the correct CTA to the Block Explorer", async () => {
		...
		const transactionHrefBlockExplorerAnchor = getByRole("link", { name: highestTransactionID });
