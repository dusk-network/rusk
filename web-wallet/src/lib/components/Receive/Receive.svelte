...
<script>
	...
	$: qrWidth = offsetHeight - receiveAddressHeight - buttonHeight
...
<div class="receive" bind:offsetHeight>
	<figure class="receive__address-qr-figure">
		...
		<figcaption class="receive__address" bind:offsetHeight={receiveAddressHeight}>
			<samp>{address}</samp>
		</figcaption>
	</figure>
	...
	<div class="receive__buttons" bind:offsetHeight={buttonHeight}>
		{#if !hideBackButton}
			<Button
				...
				on:click={() => { dispatch("operationChange", ""); }}
				text="Back"
				variant="tertiary"
			/>
		{/if}
