declare module "@dusk/w3sper" {
  ...
  abstract class BasicTransfer {
    ...
    // TODO overload? network is optional only in one case
    // maybe the base method should have no arguments instead?
    build(
      network?: Network
  ...
  abstract class RuesEvent extends Event {
    ...
    get payload(): Uint8Array | Record<string, any> | unknown; // TODO forse meglio any e buonanotte?
  }
  ...
  // TODO bisogna cambiare il tipo restituito dalla treasury
  // ma prima capire perché non abbiamo errori non avendo lo static
  // nei nostri oggetti
  // risposta: la tipizzazione è strutturale e non nominale, quindi
  // verifica solo la shape
  ...
  // TODO REMOVE IF WE USE THE ABSTRACT CLASS
  // type StakeInfo = {
  //   amount: StakeAmount | null;
  //   faults: number;
  //   hardFaults: number;
  ...
  class Bookkeeper {
    ...
    // TODO how can we avoid the union here and have a specific type?
    // TODO identifier in realtà è Key | string
  ...
  class Network {
    ...
    // TODO fill options
    static connect(url: string | URL, options?: {}): Promise<Network>;
    ...
    // TODO fill options
    connect(options?: {}): Promise<this>;

    disconnect(): Promise<void>;

    execute<T extends BasicTransfer>(tx: T): Promise<ReturnType<T["build"]>>;

    // TODO add circuits type
    prove(circuits: any): Promise<ArrayBuffer>;

    // TODO add options type
    query(gql?: string, options?: {}): Promise<Record<string, any>>;
  }
