declare module "lamb" {
	...
	function head<const L extends ArrayLike<any>> (arrayLike: L): L["length"] extends 0 ? undefined : L[0];
	...
	function insert<T> (arrayLike: ArrayLike<T>, index: number, element: T): Array<T>;

	function insertAt<T> (index: number, element: T): (arrayLike: ArrayLike<T>) => Array<T>;
	...
	function last<const L extends ArrayLike<any>> (arrayLike: L): L["length"] extends 0 ? undefined : L[-1];
	...
	function pluck<
		...
		K extends keyof U
	> (key: K): (arrayLike: ArrayLike<U>) => Array<T>;

	function pluckFrom<
		T,
		U extends Record<string, T>,
		K extends keyof U
	> (arrayLike: ArrayLike<U>, key: K): Array<T>;

	function pullFrom<
		T,
		L extends ArrayLike<T>,
		V extends ArrayLike<T>
	> (arrayLike: L, values: V): Array<T>;

	function pull<
		T,
		L extends ArrayLike<T>,
		V extends ArrayLike<T>
	> (values: V): (arrayLike: L) => Array<T>;
	...
	function rotate<
		T,
		L extends ArrayLike<T>
	> (arrayLike: L, amount: number): Array<T>;
	...
	function setAt<
		T,
		L extends ArrayLike<T>
	> (index: number, value: T): (arrayLike: L) => Array<T>;

	function setIndex<
		T,
		L extends ArrayLike<T>
	> (arrayLike: L, index: number, value: T): Array<T>;

	function shallowFlatten<
		T,
		U extends T | T[]
	> (array: Array<U>): Array<U>;
	...
	function sorter<
		T,
		RD extends SorterReader<T> = (value: T) => T,
	> (reader?: RD, comparer?: SorterComparer<T>): Sorter<T, false>;

	function sorterDesc<
		T,
		RD extends SorterReader<T> = (value: T) => T,
	> (reader?: RD, comparer?: SorterComparer<T>): Sorter<T, true>;
	...
	> (fn: F, args: Args): (...args: any[]) => ReturnType<F>;
	...
	function slice<T> (arrayLike: ArrayLike<T>, start: number, end: number): Array<T>;

	function sliceAt (start: number, end: number): <T>(arrayLike: ArrayLike<T>) => Array<T>;
