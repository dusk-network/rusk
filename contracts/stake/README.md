...
Below is a list of functions that the transfer contract made available. This contract handles staking operations, rewards, and validator management for the consensus mechanism. Each function is annotated with its access restrictions where applicable.
...
**Description**: Retrieves a reference to a stake associated with the given BLS public key. Returns None if the stake doesn't exist.
...
**Description**: Retrieves the stake keys (account and owner keys) associated with the given BLS public key. Returns None if the stake doesn't exist.
