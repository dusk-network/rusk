...
**Description**: Mints new Dusk tokens according to the withdrawal request from the stake contract. The tokens are minted either to a Phoenix address or a Moonlight account as specified in the withdrawal. This function increases the total amount of circulating Dusk and is only called during the execution of the stake contracts `withdraw` function. Any amount minted conforms to the consensus emission schedule. The tokens are minted either to a Phoenix address or a Moonlight account as specified in the withdrawal.
...
**Description**: Mints new Dusk tokens to be owned by another contract. This function increases the total amount of circulating Dusk and is intended to be called during the execution of the `withdraw_to_contract` function. Any amount minted conforms to the consensus emission schedule.
...
**Description**: Deposits funds to a contract's balance if a deposit has been placed earlier on the state.
...
**Description**: Withdraws funds from a contract's balance to either a Phoenix note or a Moonlight account. It creates either a transparent stealth note or updates an account balance.
...
**Description**: Performs an atomic conversion transacton between Phoenix notes and Moonlight balance using the deposit sent to the contract. The conversion transforms the type without changing the value.
...
**Description**: Transfers funds from one contract's balance to another and calls a function on the receiving contract (specified in the transfer argument). Contracts receiving funds are expected to expose the function specified by the sender, which is called using a `ReceiveFromContract` as argument.
...
**Description**: Returns the current root of the tree as a cryptographic commitment to the current state. The root is essential for verifying note inclusion in the merkle tree and validating phoenix transactions.
...
**Description**: Based on a given list of nullifiers, returns only the nullifiers that already exist in the contract. Nullifiers "nullify" notes and prevent double-spending attempts of already spent notes.
...
**Description**: Main entry point for all transaction executions, handling Phoenix **or** Moonlight transactions. Phoenix & Moonlight are strictly separated, and the spending phase will either go into `spend_phoenix` or `spend_moonlight` based on the transaction type. Afterwards, it executes any contract call included in the transaction. The function manages the complete transaction lifecycle from spending Dusk to executing contract calls.
...
**Description**: Adds Dusk to an account's balance, creating the account if it doesn't exist.
...
**Description**: Subtracts Dusk from an account's balance if the account exists.
